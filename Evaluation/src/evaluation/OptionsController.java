...
//               stage.showAndWait();
    ...
    public double[] calculPi(int puissance)
    {
        double[][]result = new double[size][size];
    ...
    public double[] calculPiFinal(){
        int i=0;
        double[] initial = new double[calculPi(1).length] ;
        initial = calculPi(1);
        
        while(min(calculPi(i),calculPi(i+1))>0.000001){
            i++;
          
        }
        
        initial = calculPi(i+1);
        for(int j=0;j<initial.length;j++)
            initial[j]=round(initial[j],5);
        return initial;
    }
    ...
    public double stat(double[]a,double[]b){
        double sum=0;
